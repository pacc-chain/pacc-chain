...
func main() {
	...
	rootCmd := &cobra.Command{
		Use:               "paccd",
		Short:             "EPH blockchain Daemon (server)",
	...
	// CLI command to print the paccd version
	rootCmd.AddCommand(version.Cmd())
	// CLI command to start and run the paccd (server)
	server.AddCommands(ctx, cdc, rootCmd, newApp, exportAppStateAndTMValidators)

	// prepare and add flags
	executor := cli.PrepareBaseCmd(rootCmd, "EPH", app.DefaultNodeHome)
...
func exportAppStateAndTMValidators(
	...
	if height != -1 {
		gApp := app.NewPaccApp(logger, db, traceStore, false, uint(1))
	...
	gApp := app.NewPaccApp(logger, db, traceStore, true, uint(1))
	return gApp.ExportAppStateAndValidators(forZeroHeight, jailWhiteList)
}
